    ...
    <div>
                ...
                <div class="row">
                    <div class="col text-right">
                        ...
                        <button v-on:click="[showModal('save'), cleanForm, update_status = false]" class="btn btn-sm btn-primary">
                            <i class="fas fa-fw fa-plus-square"></i> Tamdah Data Penghargaan
                        </button>
                    </div>
                </div>
                <div class="row mt-2">
                    ...
                    <ValidationObserver v-slot="{ handleSubmit }">
                        <form method="post" v-on:submit.prevent="handleSubmit(save)">
                            <div class="modal-header">
                                <h6 class="modal-title" id="saveModalLabel">Form Data Penghargaan</h6>
        ...
        <div class="modal fade" id="laporanModal" tabindex="-1" role="dialog" aria-labelledby="laporanModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h6 class="modal-title" id="myModalLabel">Daftar Penghargaan</h6>
                    ...
                    <div class="modal-body text-center">
                        <vue-excel-editor v-model="rows" ref="grid" readonly>
                            ...
                            <vue-excel-column field="deskripsi" label="Deskripsi" width="400px" />
                        </vue-excel-editor>
                    </div>
                    <div class="modal-footer">
...
export default {
    ...
    methods: {
        async lists () {
            ...
            await axios.get('/penghargaan/lists')
            .then(response => {
                this.rows = response.data;
                loader.hide();
                this.$toastr.success('Pengambilan data <b>Penghargaan</b> berhasil.');
            }).catch(error => {
                loader.hide();
        ...
        async save () {
            ...
            if (this.update_status == false) {
                await axios.post('/penghargaan/store', this.form)
                .then(response => {
                    if (!response.data.status) {
                        ...
                        this.hideModal('save');
                        loader.hide();
                        this.$toastr.success('Data <b>Penghargaan</b> berhasil ditambahkan.');
                    } else {
                        this.cleanForm();
                        this.hideModal('save');
                        loader.hide();
                        this.$toastr.info('Data <b>Penghargaan</b> sudah tersedia.');
                    }
                }).catch(error => {
                    loader.hide();
                ...
                await axios.post('/penghargaan/update/' + this.penghargaan_id, this.form)
                .then(response => {
                    if (!response.data.status) {
                        let index = this.rows.findIndex(x => x.id == this.penghargaan_id);
                        this.rows[index].id = this.penghargaan_id;
                        ...
                        this.hideModal('save');
                        loader.hide();
                        this.$toastr.success('Data <b>Penghargaan</b> berhasil Diubah.');
                    } else {
                        this.hideModal('save');
                        loader.hide();
                        this.$toastr.info('Data <b>Penghargaan</b> sudah tersedia.');
        ...
        async destroy (id) {
            let loader = this.$loading.show();
            await axios.get('/penghargaan/destroy/' + id)
            .then(response => {
                if (response.status == 'relasi tersedia') {
                    loader.hide();
                    this.$toastr.info('Beberapa data <b>Penebusan</b> terkait data ini.');
                } else {
                    let index = this.rows.findIndex(x => x.id == id);
                    this.rows.splice(index, 1);
                    loader.hide();
                    this.$toastr.success('Data <b>Penghargaan</b> berhasil dihapus.');
                }
            }).catch(error => {
                loader.hide();
        ...
        edit (id) {
            let penghargaan = this.rows.find(x => x.id == id);
            this.form.kode = penghargaan.kode;
            this.form.nilai = penghargaan.nilai;
            this.form.deskripsi = penghargaan.deskripsi;
            this.update_status = true;
            this.penghargaan_id = id;
            this.showModal('save');
        },
        confirmDestroy (id) {
            this.$swal({
                title: 'Apakah Anda Yakin ?',
                text: 'Data penghargaan akan dihapus permanen.',
